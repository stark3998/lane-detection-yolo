...
class FRAME :
    ...
    def update_trackers(self, img):
        ...
        if self.count % self.__yp == 0:
            # Assuming `obstructions` is defined somewhere
    ...
    def process_video(self, file_path, fps_factor, \
        ...
        total_duration = nb_frames / fps_actual
        print("{:s} Number of Frames : {:d} WIDTH {:d} HEIGHT {:d} FPS {:.2f} DUR {:.1f} s".format(\
            file_path, nb_frames, frame_w, frame_h, fps_actual, nb_frames // fps_actual
        ))

        if pers_frame_time > total_duration:
            print(f"Error: pers_frame_time ({pers_frame_time} s) exceeds the video duration ({total_duration:.1f} s).")
            return
        ...
        # pers_frame_time_ms = pers_frame_time * 1000  # Convert seconds to milliseconds
        # video_reader.set(cv2.CAP_PROP_POS_MSEC, pers_frame_time_ms)

        # Read the frame and check if successful
        ret, self.image = video_reader.read()
        if not ret or self.image is None:
            print("Error: Could not read frame at specified time.")
            return
        ...
        for i in tqdm(range(int(t0 * fps_actual), int(t1 * fps_actual)), mininterval=3):
            ...
            if status and image is not None and (i % fps_factor == 0):
...
if __name__ == "__main__":
    ...
    frame.process_video(file_path, 1,\
            video_out = video_out,pers_frame_time =10,\
            t0  =0 , t1 =150)#None)
