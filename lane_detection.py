...
class OBSTACLE(BoundBox):
        ...
        def update_coord(self, box):
            ...
            if len(self.position_hist) > 0:  # Check if position_hist is not empty
...
class LANE_DETECTION:
    ...
    def __init__(self,  img,fps,
        ...
        self.window_height = np.int64(self.UNWARPED_SIZE[1]//self.windows_per_line)
    ...
    def calc_perspective(self,  lane_start=[0.25,0.75]):
        ...
        if self.verbose  > 1:       
            ...
            cv2.circle(img_orig, tuple(map(int, self.vanishing_point)), 10, color=RED, thickness=5)
            cv2.circle(img_orig, tuple(map(int, orig_points)), 10, color=GRAY, thickness=4)
